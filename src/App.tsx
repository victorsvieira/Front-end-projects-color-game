...
type GameState = { points: number; mistakes: number; mistakesInARow: number; gameMode: string };
type GameAction = { type: "CORRECT" } | { type: "INCORRECT" } | { type: "RGB" } | { type: "HEX" } | { type: "HSL" };

export const GameContext = createContext<{ state: GameState; dispatch: React.Dispatch<GameAction> }>({
  state: { points: 0, mistakes: 0, mistakesInARow: 0, gameMode: "RGB" },
  dispatch: () => {},
});
...
function App() {
  const [state, dispatch] = useReducer(reducer, { points: 0, mistakes: 0, mistakesInARow: 0, gameMode: "RGB" });
  return (
